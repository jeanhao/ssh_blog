...
<div class="container clearfix">
    ...
    <div class="main-wrap">
        <div class="crumb-wrap">
            ...
            &gt;</span><span class="crumb-name"><a href="user/userlist.action?pageNow=1">用户管理</a></span>
                ...
                <div class="result-content">
                    <table class="result-tab" width="100%">
						...
						<tr style="background-color:white">
							...
							<td width="7%"><a href="user/update.action?id=<c:out value="${user.id}"/>">修改</a></td>
							<td width="7%"><a href="user/delete.action?id=<c:out value="${user.id}"/>"onClick="if(!confirm('确定删除该信息吗？'))return false;else return true;">删除</a></td>
						</tr>
						</c:forEach>
					</table>
					...
					<font size="2">条记录&nbsp;&nbsp;
					<!-- 首页始终显示第一页 -->
					<a href="user/userSearch.action?pageNow=1&keywords=${keywords}&SearchType=${SearchType}&informationType=${informationType}">首页</a>
					<!-- 如果有前一页就提交前一页的pageNow值 -->
					<c:choose>
						<c:when test="${page.hasPre}">
							<a href="user/userSearch.action?pageNow=${page.pageNow-1}&keywords=${keywords}&SearchType=${SearchType}&informationType=${informationType}">上一页</a>
						</c:when>
						<c:otherwise>
							<a href="user/userSearch.action?pageNow=1&keywords=${keywords}&SearchType=${SearchType}&informationType=${informationType}">上一页</a>
						</c:otherwise>
					</c:choose>
					<c:choose>
						<c:when test="${page.hasNext}">
							<a href="user/userSearch.action?pageNow=${page.pageNow+1}&keywords=${keywords}&SearchType=${SearchType}&informationType=${informationType}">下一页</a>
						</c:when>
						<c:otherwise>
							<a href="user/userSearch.action?pageNow=${page.totalPage}&keywords=${keywords}&SearchType=${SearchType}&informationType=${informationType}">下一页</a>
						</c:otherwise>
					</c:choose>
					<!-- 尾页始终提交最后一页的pageNow值 -->
					<a href="user/userSearch.action?pageNow=${page.totalPage}&keywords=${keywords}&SearchType=${SearchType}&informationType=${informationType}">尾页</a>
					&nbsp;跳转到:
					</font>
					<form name="myForm" action="user/userSearch.action" method="get" style="display:inline;">
